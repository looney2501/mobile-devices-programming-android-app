package com.ilazar.myapp.hotel.ui.item
...
class ItemViewModel(private val itemId: String?, private val itemRepository: ItemRepository) :
    ...
    fun saveOrUpdateItem(foodName: String, price: Int, dateBought: String, onSale: Boolean) {
        viewModelScope.launch {
            Log.d(TAG, "saveOrUpdateItem...");
            try {
                uiState = uiState.copy(isSaving = true, savingError = null)
                val item = uiState.item?.copy(name = foodName, capacity = price,
                    dateRegistered = dateBought, isAvailable = onSale)
                if (itemId == null) {
