...
import com.ilazar.myapp.todo.util.MyNetworkStatusViewModel
...
fun LoginScreen(onClose: () -> Unit) {
    ...
    val myNewtworkStatusViewModel =
        viewModel<MyNetworkStatusViewModel>(factory = MyNetworkStatusViewModel.Factory(LocalContext.current.applicationContext as Application))
    onlineStatus = if (myNewtworkStatusViewModel.uiState) {
        "Online"
    } else {
        "Offline"
    }
    Scaffold(
        topBar = {
            TopAppBar(title = { Text(text = stringResource(id = R.string.login)) },
                actions = { Text(text = onlineStatus); })
        }) {
        Column(
            modifier = Modifier
                .fillMaxSize()
